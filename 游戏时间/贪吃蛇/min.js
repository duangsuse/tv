...
清空=(a,f)=>{ let i=0,N=a.length,nF=f.length; a=a.splice(0,N);for(;i<N;i+=nF)f(...a.slice(i,i+nF)) } //仅贴图方块

var RL,ps
蛇=(n,m,阵)=>{ var p,d=1,  d新=[],f=d1=>{if(d+d1)d新.push(d1)},c, 选=n=>5+rand(n-5)
  ...
  if(cmd) (RL=RL||require("repl").start()).input.on("keypress", (_,ev)=>f( (c=ev.code)? [-m,+m,+1,-1] [c.charCodeAt(1)-0x41]:-d) );
  else{window.onkeydown=ev=>f([-1,-m,+1,+m] [ev.keyCode-37/*左键*/]);
  for(let kev of ss("mouse touch"))
    触向(window,kev,(x0,y0,x,y,ev)=>{ x-=x0;y-=y0; let q=abs(x)>abs(y), v=q?1:m, d=q?x:y; if(abs(d)>2)ev.preventDefault(); f(d<0? -v:v) })
  }
  步=()=>{
    清空(d新,v=>d=v);p=p+d; if((c= 阵[p])!=0&&c<3) 死掉(); else { 
...
gid=0, 配={绘:0, 复活:0, 界:[20,30], 贴文:"　蛇墙果头尾",贴色:ss("gray #66ccff green red blue #aaaaaa")},//grid/字符; 单次,重开,加分,不死; 高宽
主循环=f=>{let[n,m]=配.界,N=n*m, a,ae, [init,go]=f(n,m,a=Array(N)),dt=1000/7 //秒格速度 ;可用 n=cmd.stdout.rows 或 URLSearchParams("w=1").get
  ...
  let 画文=cmd? s=>{so.write("\x1b[H");so.write(s)} : s=>{doc.body.innerText=s}, so=o=cmd?cmd.stdout:0, {贴色,贴文}=配, 旧p=[],
  绘1=()=>清空(旧p, (p,v0)=>a[p]=v0),绘0=()=>{let N=ps.length; if(N==1)return;
    [ps[N-1],ps[0]].forEach((p,i)=>{ 旧p.push(p,a[p]); a[p]=4+i })//头
  }

  绘=cmd||配.绘==1? ()=>{let p, j=0,s=[]; 绘0()
