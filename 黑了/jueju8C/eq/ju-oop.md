...
《跳转回填 backpatcher》
“编程是什么？”  “规范记录自己想过的解法呗”？无论规范出对象还是函数，被解释或被翻译，背靠有什么了不得的代数，社区里都有 How < What < Why 三等级

How 只关心眼前的流程或算法，即便它的API或亮点一句话就能说清，或者干脆照古猫画今虎， 好比热衷笔法的画匠，“静态编译化脚本小子”
...
在已能回答出WHY的一刻，代码便从该“积栈的”符文，化作写字念书的母语，只会因创作与设计而产生！
好比漫画家。穷其变，悟不穷，以明我志。
但奇怪的是，这反而和傻瓜用户、和极客菜鸟没什么区别？

不如学院派能知其变，守其恒，为天下式。
知识这东西不单调。你讲的少，人不给你流量和投资；讲太清了，暴露出跨域的创作接口，不就把咱的天才给稀释了？

但元编程和体操说到底还是编程。编程也还是有微调的“查询与收纳”。 位置是相对的，时代与文明却如同锚点。专业知识和八股文，只有越来越不值钱，才会像Pythonic那样启发值钱的事物
已跳出这段易问难解的过程式，才能看清它还给你的价值吧？ 


HOLPful 的程序员认为
- 文档与抽象，应泛达雅：泛用、易举例、易巧记。lib代码的复杂性应当被拆解换名，直到简明。可读性不朽，“小”聪明难久
- 程序员“开销”>CPU开销, 让numpy变快>让gcc -O3 变快
- 用户耗时>脚本耗时，好记的手势和快捷键>完善的类型签名：语意直白>逻辑准确
- 作为框架不能说explicit优于implicit。负起穷举用法模型的责任，务求“不必用=不用学”
- 亮点>示例API>测试>代码。编程只是快乐的涂鸦，电脑和学科社区的展现权 只限于让涂鸦等效生产环境
- 别怀念ifelse链和class单例命名空间。
- 别在模型前，怀念学到的特例；即便需求繁多，错误不能被糊弄，API也无模棱两可的定价权
- 只准有一个……至少只封装出一种API，那么写明的提问就是最佳答案
- 哪怕你搜到的，一开始还不够Pythonic。快速原型很实干，但并非不选型就打字蛮干
- 讲不清文档的代码，一定是0模型的拼凑品
- 简单自然的代码，不一定是0功能的入门品。 用户叙事性>协作力>纯技巧
- 不要把类型或参数值前后缀到函数名类名，给重载和多态留一点导入空间
- 代码的设计是，想要加的功能越多，易写易记的程度越大。无为而作，少即是多，使用频率即正义
- 链式访问和加减只是调用，class只是共变量函数。编程语言们总是有更棒的灵感！
分别是这些原则:
泛达雅 开销在于算法 瓶颈在于UX 语境优于严谨 组合优于专业
正确归类惯用法 最小意外 单一职责 自顶向下 防面条代码 防过度工程 0冗余化命名 学习即压缩 元编程是编程
