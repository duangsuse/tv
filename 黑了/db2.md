...
你可以用 LIKE "%发财" 匹配 `"张发财","李发财"` ，而 LIKE "_某" 匹配字串 吴某 范某 等某
...
别忘了过度脚本化，曾造成SQL注入等严重问题的PHP 2022年了还有人用呢。JavaEE log4j 2021 年了还出严重RCE代码执行呢，还复古流行起 CRUD boy 用"sqlmapper" Mybatis 绑定SQL语句去执行呢……

__因为用途领域上的尴尬，SQL的用户接口真的差劲极了__ 。免费的 PHPMyAdmin 查点小东西都要重载、Navicat啥的收费又复杂，HeidiSQL, DBeaver 这些也都不适合入门 ，MSOffice 一堆“外行”好用但不会做普通SQL的“支持”，更何谈有大部分院校还在教学生用 CLI(cmd.exe) 客户端 `mysql -u root -p` 从记事本右键粘贴啦

然而，宝刀未老的SQL仍是大部分人的选择。作者也是出于娱乐的心态弄了一门翻译到SQL的小语言，并支持用 ?0 和 `$新用户=新(?0 ?1 ?2)用户(默认)` 这种语法创建“函数”，当然在这里，这个为语言API桥接 的特性没有用处。只是说SQL真的很鸡肋

IT工程界和教育有这么不堪吗？很抱歉，的确有。无论自诩多高等的机构、高级工程师，排名顶尖的，的确有。
...
>SQ糖里 `新("电子字典" 无)物品(名="字典")` 代表 `新 名="电子"/*无改主人*/ 物品()` 或 `default`，而空代表NULL
...
SQL 的功能实际上是数据统计而非1:N存储和 FilerSort 。它甚至在某些数据上比 Excel 好用，比如单位换算和表头(列名) SQL都能处理
...
组内排序需要 group_concat ，这个SQLite里也有，不是MySQL/Postgres的扩展
...
也可以在DevTools 应用>存储(F12>Application>Storage)>WebSQL 查看表与执行SQL

若不看注释， `SQ糖=SQ糖糖(()=>[])` 即可

SQ糖提供了较简洁的语法，可在js代码里查看
